function OurRequest(data, commands)
{
	...
	if (urlFirstSlashInd == -1)
	{
		...
		this.urlCommand = [this.path.slice(1)];
	...
	for(var i = 1; i < lines.length; i++ )
	{
		...
		if (currLine[0] == "Host")
		{
			...
			{
				this.host = currLine[1].trim();//host="www.tutorialspoint.com"
			}
			else
			{
				var indOfColon =  currLine[1].indexOf(':'); //Host: "example.com:3000"
				this.host = currLine[1].substring(0, indOfColon).trim(); //Host: "example.com:3000"
			}
		}		

		if (currLine[0] == "Cookie")
		{
			cookieList =  currLine[1].trim();
			cookieList = cookieList.split(';');
			for (var i = 0; i < cookieList.length; i++)
			{
				cur = cookieList[i];
				curSplitted = cur.split('=');
				key = curSplitted[0].trim();
				val = curSplitted[1].trim();
